...
> 4 个阶段 · 14 节课程 + 3 个实战项目 · 纯 HTML/CSS/JS，双击 `index.html` 即用。
...
- 📚 **14 节完整教程 + 3 个实战项目**：概念讲解 + 代码示例（含预期输出）+ 排错表 + 小练习 + 随堂小测（57 题）
- 🛠 **实战项目按插件组织**：Hello World → 状态面板 → 错误记录器，每个 = 项目页（步骤条 + 逐步进度）；新增插件 = 在 `projects.js` 追加一个对象（见 `practice/_TEMPLATE.md`）
- 🧭 **课程总览首页**：进度条、继续学习定位、四阶段导航
- 🔍 **全文搜索**：输入关键词（如 `EADDRINUSE`）直达相关课程并高亮
- 📖 **术语表**：40 个黑话速查，可跳回出处课程
- 👀 **访问统计**：页尾展示网站访问量（第三方服务，失败自动隐藏）
- 📝 **学习进度**：课程级 + 项目步骤级双层进度，自动存 localStorage
...
| 三 · 实战演练（1 课 + 项目库） | 实战方法论（四步闭环）+ 3 个实战项目：Hello World → 状态面板 → 错误记录器（每个 = 设计 → 骨架 → 逻辑 → 验证 → 配置） | 写出并跑通自己的插件 |
...
└── js/
    ...
    └── content/          # 课程内容（以后加课/改课只动这里）
        ...
        ├── content-s3.js     # 阶段三方法论总览课
        ├── content-s4.js     # 阶段四课程内容
        └── practice/         # ★ 实战项目库
            ├── projects.js   #   项目数据（顺序即展示顺序）
            └── _TEMPLATE.md  #   新增项目模板说明
```
