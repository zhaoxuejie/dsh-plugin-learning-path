...
> 4 个阶段 · 14 节课程 + 实战项目库 · 纯 HTML/CSS/JS，双击 `index.html` 即用。
...
- 📚 **14 节完整教程 + 实战项目库**：概念讲解 + 代码示例（含预期输出）+ 排错表 + 小练习 + 随堂小测（47 题）
- 🛠 **实战项目按插件组织**：每个插件 = 一个项目页（步骤条 + 逐步进度），以后加插件只加一个文件
...
| 三 · 实战演练（1 课 + 项目库） | 实战方法论（四步闭环）+ 实战项目（如 error-logger：设计 → 骨架 → 逻辑 → 验证 → 配置） | 写出并跑通自己的插件 |
...
└── js/
    ...
    └── content/          # 课程内容（以后加课/改课只动这里）
        ...
        └── practice/         # ★ 实战项目库（每个插件一个文件，可不断扩充）
            └── error-logger.js   # error-logger 错误记录器（5 步骤）
```
